...
func (suite *ControllerTestSuite) TestSkipCalendarSyncManagedSourceEvents() {
	ctx := context.Background()
	startTime := time.Now()
	endTime := startTime.Add(2 * time.Hour)
	...
	sourceEvents := []models.Event{
		{
			...
			Description: "generated by CalendarSync earlier",
			StartTime:   startTime,
			EndTime:     endTime,
			Metadata: &models.Metadata{
				SyncID:   "managed-sync-id",
				SourceID: "someOtherCalendar",
				Managed:  true,
			},
			Accepted: true,
		},
